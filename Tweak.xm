...
static NSString *stringForObject(id object, NSString *prefix,
  ...
  if (!object) {
    ...
    for (id val in array) {
      str = XStr(@"%@\n%@\t%@", str, prefix,
                 stringForObject(val, XStr(@"%@\t", prefix), dontTruncate));
    }
    str = XStr(@"%@\n%@]", str, prefix);
...
static void addToLogIfEnabled(NSString *service, BBBulletin *bulletin,
                              NSString *label, id object, BOOL dontTruncate) {
  // allow global service which is @"" so empty
  if (!XIsEmpty(service) && pusherEnabledLogs &&
      ![pusherEnabledLogs containsObject:service]) {
...
static PusherAuthorizationType getServiceAuthType(NSString *service,
                                                  NSDictionary *servicePrefs) {
  if (servicePrefs[@"isCustomService"] &&
      ((NSNumber *)servicePrefs[@"isCustomService"]).boolValue) {
...
static UIImage *shrinkImage(UIImage *image, CGFloat factor) {
  ...
  // initWithSize:newSize]; return [renderer
...
static void pusherPrefsChanged() {
  ...
  pusherWhenToPush =
      val ? ((NSNumber *)val).intValue : PUSHER_WHEN_TO_PUSH_EITHER;
  val = prefs[@"WhatNetwork"];
  pusherWhatNetwork =
      val ? ((NSNumber *)val).intValue : PUSHER_WHAT_NETWORK_ANY;
  ...
  for (NSString *service in customServices.allKeys) {
    ...
    for (NSString *customAppID in prefCustomApps.allKeys) {
      ...
      // key doesn't exist COMMENTED OUT BECAUSE REMOVED ENABLE/DISABLE STATUS
      // FOR CUSTOM APPS if (customAppPrefs[@"enabled"] && !((NSNumber *)
      // customAppPrefs[@"enabled"]).boolValue) { 	continue;
      // }
      if (!customAppPrefs) {
        continue;
      }
  ...
  for (NSString *service in BUILTIN_PUSHER_SERVICES) {
    ...
    servicePrefs[@"whatNetwork"] =
        val ?: @(pusherWhatNetwork); // if not set, go with default
    servicePrefs[@"whatNetwork"] =
        [(((NSNumber *)servicePrefs[@"whatNetwork"]).intValue ==
                  PUSHER_SEGMENT_CELL_DEFAULT
              ? @(pusherWhatNetwork)
    ...
    // default is service disabled
    if (prefs[enabledKey] == nil ||
        !((NSNumber *)prefs[enabledKey]).boolValue) {
      // skip if disabled
      [pusherEnabledServices removeObjectForKey:service];
    } else {
      pusherEnabledServices[service] = pusherServicePrefs[service];
...
static NSString *snsSaysNo(NSArray *sns, BBSectionInfo *sectionInfo, BOOL isAnd,
                           BOOL requireANWithOR) {
...
static NSString *prefsSayNo(BBServer *server, BBBulletin *bulletin) {
  ...
  if (!pusherEnabledServices ||
      ![pusherEnabledServices isKindOfClass:NSDictionary.class]) {
    return XStr(@"Enabled services is nil, it shouldn't be. %@",
                pusherEnabledServices);
  }
...
- (void)sendBulletinToPusher:(BBBulletin *)bulletin {
  ...
  // bulletin.lastInterruptDate is nil upon respring, so ignore if that's the
  // case
  if (!bulletin.lastInterruptDate) {
    ...
    addToLogIfEnabled(
        @"", bulletin,
        @"Last interrupt date nil (this should only happen if SpringBoard just "
  ...
  // THIS METHOD IS BAD BECAUSE THEN NOTIFICATIONS DATED IN THE PAST (e.g.
  // Messages, Mail, etc.) WILL NOT GET FORWARDED THIS IS WHY USE NEW METHOD OF
  // lastInterruptDate = nil ABOVE
  // // Check if notification within last 5 seconds so we don't send uncleared
  // notifications every respring NSDate *fiveSecondsAgo = [[NSDate date]
  // dateByAddingTimeInterval:-5]; if (bulletin.date && [bulletin.date
  // compare:fiveSecondsAgo] == NSOrderedAscending) { 	XLog(@"Bulletin 5
  // seconds or older"); 	addToLogIfEnabled(@"", bulletin, @"Notification
  // dated greater than 5 seconds ago, not sending to prevent resending all
  // notifications on respring"); 	return;
  // }
  ...
  BOOL appListContainsApp =
      [globalAppList containsObject:appID.lowercaseString];
  if (globalAppListIsBlacklist == appListContainsApp) {
    XLog(@"[Global] Blocked by app list: %@", appID);
    addToLogIfEnabled(
        @"", bulletin,
        XStr(@"Blocked by global app list (%@)",
             globalAppListIsBlacklist ? @"blacklist" : @"whitelist"));
    return;
  }
  ...
  for (NSString *service in pusherEnabledServices.allKeys) {
    [self sendToPusherService:service
                     bulletin:bulletin
                        appID:appID
                      appName:appName
                        title:title
...
- (void)sendToPusherService:(NSString *)service bulletin:(BBBulletin *)bulletin appID:(NSString *)appID
  ...
  if (!isTest) {
    ...
    if (appListIsBlacklist &&
        ((NSNumber *)appListIsBlacklist).boolValue == appListContainsApp) {
      XLog(@"[S:%@] Blocked by app list: %@", service, appID);
      addToLogIfEnabled(service, bulletin,
                        XStr(@"Blocked by app list (%@)",
                             ((NSNumber *)appListIsBlacklist).boolValue
                                 ? @"blacklist"
                                 : @"whitelist"));
      return;
    }
    ...
    BOOL isAnd = servicePrefs[@"snsIsAnd"] &&
                 ((NSNumber *)servicePrefs[@"snsIsAnd"]).boolValue;
    BOOL requireANWithOR =
        servicePrefs[@"snsRequireANWithOR"] &&
        ((NSNumber *)servicePrefs[@"snsRequireANWithOR"]).boolValue;
    BBSectionInfo *sectionInfo =
        [self _sectionInfoForSectionID:bulletin.sectionID effective:YES];
    if (!sectionInfo) {
      XLog(@"[S:%@,A:%@] sectionInfo nil", service, appID);
      addToLogIfEnabled(service, bulletin,
                        @"sectionInfo is nil, it should not be");
      return;
    }
    ...
    int serviceWhatNetwork =
        ((NSNumber *)servicePrefs[@"whatNetwork"]).intValue;
    XLog(@"[S:%@,A:%@] Doing Device Conditions", service, appID);
    NSString *deviceConditionsResponse =
        deviceConditionsSayNo(serviceWhenToPush, serviceWhatNetwork);
  ...
  NSNumber *includeIcon =
      servicePrefs[@"includeIcon"]
          ?: @NO; // default NO for custom services, default check for built in
                  // services done earlier so should never get to this NO
  NSNumber *includeImage =
      servicePrefs[@"includeImage"]
          ?: @NO; // default NO for custom services, default check for built in
                  // services done earlier so should never get to this NO
  NSNumber *curateData = servicePrefs[@"curateData"] ?: @YES;
  NSNumber *imageMaxWidth =
      servicePrefs[@"imageMaxWidth"] ?: @(PUSHER_DEFAULT_MAX_WIDTH);
  NSNumber *imageMaxHeight =
  ...
  NSDictionary *infoDict = [self
      getPusherInfoDictionaryForService:service
                         withDictionary:@{
                           @"title" : title ?: @"",
                           @"message" : message ?: @"",
                           @"devices" : devices ?: @[],
                           ...
                           @"dateFormat" : XStrDefault(
                               servicePrefs[@"dateFormat"], @"MMM d, h:mm a"),
...
- (NSDictionary *)getPusherInfoDictionaryForService:(NSString *)service withDictionary:(NSDictionary *)dictionary {
  ...
  if (dictionary[@"includeIcon"] &&
      ((NSNumber *)dictionary[@"includeIcon"]).boolValue) {
    data[@"icon"] = [self base64IconDataForBundleID:bulletin.sectionID];
  }

  if (dictionary[@"includeImage"] &&
  ...
  if (XEq(service, PUSHER_SERVICE_IFTTT)) {
    if (dictionary[@"curateData"] &&
      ...
      return @{
        @"value1" : dictionary[@"title"],
        @"value2" : dictionary[@"message"],
...
- (NSDictionary *)getPusherCredentialsForService:(NSString *)service withDictionary:(NSDictionary *)dictionary {
  if (XEq(service, PUSHER_SERVICE_PUSHOVER)) {
    ...
    return @{@"headerName" : @"Access-Token", @"value" : dictionary[@"token"]};
  } else if (XEq(service, PUSHER_SERVICE_IFTTT)) {
  ...
  if (dictionary[@"paramName"] &&
      ((NSString *)dictionary[@"paramName"]).length > 0) {
    return @{dictionary[@"paramName"] : dictionary[@"key"]};
  } else if (dictionary[@"headerName"] &&
             ((NSString *)dictionary[@"headerName"]).length > 0) {
    return @{
        ...
        completionHandler:^(NSData *data, NSURLResponse *response,
          ...
          if (requestSucceeded) {
            ...
            if (image &&
              ...
              // if last retry and has image, set image property to true instead
              // of image base64
              ...
              addToLogIfEnabled(
                  service, bulletin,
                  XStr(@"----- Retrying. Try %d of %d with image %@%@. Network "
                       @"Response: Success, but response was %@. -----",
            ...
            if (retriesLeft) {
              if (retriesLeft.intValue > 0) {
                ...
                if (retryInfoDict[@"image"] && pusherRetriesLeft[retryKey] &&
                    ((NSNumber *)pusherRetriesLeft[retryKey]).intValue == 0) {
                  retryInfoDict[@"image"] = @YES;
                }
                ...
                addToLogIfEnabled(
                    ...
                    XStr(@"----- Retrying. Try %d of %d -----",
                         PUSHER_TRIES - (retriesLeft.intValue - 1),
                         PUSHER_TRIES));
