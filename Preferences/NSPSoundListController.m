...
- (void)viewWillAppear:(BOOL)animated {
  ...
  for (int i = 0; i < _serviceSounds.count; i++) {
    indexesToReplace[@(i)] = [_serviceSounds[i] mutableCopy];
  }
  for (NSNumber *index in indexesToReplace.allKeys) {
    [_serviceSounds replaceObjectAtIndex:index.intValue
...
- (void)saveServiceSounds {
  if (_isCustomApp) {
    ...
    setPreference((__bridge CFStringRef)_prefsKey,
                  (__bridge CFArrayRef)_serviceSounds, YES);
  }
}
...
- (NSArray *)specifiers {
  if (!_specifiers) {
    ...
    if (_serviceSounds.count) {
      ...
      // notifications to all sounds." forKey:@"footerText"]; } else if
      // (XEq(_service, PUSHER_SERVICE_PUSHBULLET)) { 	[groupSpecifier
      // setProperty:@"Pushbullet only allows one receiving sound. Selecting
      // none will forward push notifications to all sounds."
      // forKey:@"footerText"];
      // }
    ...
    for (NSDictionary *sound in [self sortedSoundList:_serviceSounds]) {
      PSSpecifier *switchSpecifier = [PSSpecifier
          preferenceSpecifierNamed:sound[@"name"]
                            target:self
                               set:@selector(setPreferenceValue:
                                              forSoundSpecifier:)
                               get:@selector(readSoundPreferenceValue:)
                            detail:nil
                              cell:PSSwitchCell
                              edit:nil];
      switchSpecifier.identifier = sound[@"id"];
      [switchSpecifier setProperty:@"com.noahsaso.pusher/prefs"
...
- (NSArray *)sortedSoundList:(NSArray *)sounds {
  return [sounds sortedArrayUsingComparator:^NSComparisonResult(
                     NSDictionary *sound1, NSDictionary *sound2) {
    return [sound1[@"name"] localizedCaseInsensitiveCompare:sound2[@"name"]];
  }];
}

- (void)setPreferenceValue:(id)value
         forSoundSpecifier:(PSSpecifier *)specifier {
